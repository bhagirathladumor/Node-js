...
module.exports.otpPage = async (req, res) => {
    try {
        if (!req.session.OTP || !req.session.resetUserId) {
            return res.redirect('/');
        }
...
module.exports.VerifyOtp = async (req, res) => {
    try {
        if (req.body.OTP !== req.session.OTP || Date.now() > req.session.otpExpiry) {
            return res.render('auth/otpPage', { error: 'Invalid or expired OTP. Please try again.' });
        }
